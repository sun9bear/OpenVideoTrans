...
class WorkerConfig:
    ...
    # post-HEAD swap). The per-mode duration caps split by binding constraint: subtitle-only is
    # bound by cloud ASR/MT, anything producing dubbed audio by the tighter TTS wall-time.
    max_upload_bytes: int = 500 * 1024 * 1024
    max_video_duration_dub_sec: int = 300  # dub_only + both (TTS wall-time bound)
    max_video_duration_subtitle_sec: int = 1800  # subtitle_only (cloud ASR/MT bound)

    def duration_cap_sec(self, output_mode: str) -> int:
        """The hard duration cap (seconds) for an output_mode (plan §13).

        subtitle_only gets the looser cloud-ASR/MT cap; every other mode produces dubbed audio and
        is bound by the tighter TTS wall-time. An unknown mode falls back to the tightest cap
        (fail-safe) rather than an unbounded default.
        """
        return (
            self.max_video_duration_subtitle_sec
            if output_mode == "subtitle_only"
            else self.max_video_duration_dub_sec
        )
