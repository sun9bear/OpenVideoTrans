...
def _tts_pin_serviceable(provider: str, target_lang: str) -> bool:
    """Whether an EXPLICITLY user-pinned TTS ``provider`` can structurally serve ``target_lang`` on
    THIS box — deliberately BYPASSING the commercial-safe gate (plan §8 decision 4: a user may pin
    the experimental, non-commercial edge_tts voice; the owner bears the ToS risk, and edge is still
    never auto-routed). Structural = (a) the provider's adapter is available (installed / keyed, via
    probe) AND not paid — the paid red line still holds even for a pin; (b) the capability registry
    lists the provider as covering the locale; and (c) for piper, the installed model actually
    covers the locale. A miss here is a STRUCTURAL gap the picker should never have offered, so the
    worker fails the job closed (``tts_provider_unavailable``) rather than silently substituting —
    distinct from a transient run-time exhaustion, which reroutes + flags voice_substituted."""
    ...
    if cap is None or provider not in cap.tts_models:
        return False
    return provider != "piper" or piper_model_covers(target_lang)
...
def run_real_pipeline(
    ...
    *,
    ...
    tts_pin = job.plan.tts if ("tts" in kinds and job.plan.tts_voice and job.plan.tts) else None
    tts_voice = job.plan.tts_voice if tts_pin else None
    voice_substituted = False
    if tts_pin is not None and not _tts_pin_serviceable(tts_pin, job.target_lang):
        raise LanguageError(
    ...
    for _ in range(max_reroutes + 1):
        ...
        # don't re-substitute or resurrect the dead voice id (wrong for the new engine).
