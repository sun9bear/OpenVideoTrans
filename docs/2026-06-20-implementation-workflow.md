...
**状态：** **执行阶段（i18n 闸已于 2026-06-23 由项目主开启 → STEP0 起产品码实施已授权，按本规程自主推进至 M1）**。M1→M2/M2.1/M3 等里程碑边界仍待项目主 sign-off（§8）。历史：i18n 完成前只设计 + 验证机器、不写产品码。
**日期：** 2026-06-20
**配套：** 执行单元见 [backlog v2.1](2026-06-20-tier1-implementation-backlog.md)（29 单元 + DAG）；GitHub issue #1–#29 + EPIC #30。

> **硬前提（已满足）：** i18n 闸已于 2026-06-23 由项目主开启，STEP0+ 产品码实施已授权（母文档 §6 执行顺序门）。历史：i18n 完成前只可定规程 / 验证机器、不写产品码（含 STEP0）。
