...
> **云 ASR provider 限制 + 切块（v4，CodeX P1.2）**：Groq STT 非 chunking 上传上限 **25MB**（官方）、且 Whisper Turbo 有按小时定价口径 → 视为**免费配额 provider**、非绝对 $0。**必须 `asr_chunker`**：抽 16k mono 音频 → 按 provider `max_bytes`/`max_duration` 切块 → 各块转录后**按 offset 合并 transcript**（内核已有 CF 分块 `FVD_CF_CHUNK_SEC`，泛化到 groq 字节上限）。**否则 30min 字幕-only（头号功能）实际跑不通**（30min/16k mono ≈ 57MB > 25MB）。平台限额实施时现查（§13 反漂移）。
...
- **云 ASR + 切块（v4，CodeX P1.2）**：transcribe 走云（groq→CF）；**`asr_chunker`** 抽 16k mono → 按 provider `max_bytes`/`max_duration` 切块 → 各块转录后 offset 合并 transcript（内核 CF 分块泛化到 groq 25MB 字节上限，§5）；长视频字幕-only 靠此跑通。
...
| 模型/语言（v4） | `language_capabilities` registry（每 locale：mt/subtitle/tts_supported · tts_models · burn_font · default_voice · license_status · quality_tier；**按 output_mode 分层启用**）· `asr_default` · `asr_chunker`(max_bytes/duration) · `no_model_policy` · **`model_cache`（懒加载到持久卷开关/容量上限）** · **`target_languages`（起步首批启用集）** |
