...
>
> **Groq 暂不可用的临时 ASR 策略（2026-06，prep 阶段 Groq 注册受阻 `signup error`，疑地区风控）**：Groq 是云 ASR 主力池（~2880 audio-min/天）；**未注册成功前** ASR = **`cloudflare`(Workers AI Whisper) 主**（~214 audio-min/天）→ **`faster_whisper` 本地兜底**（hosted 临时**重新 bake** faster-whisper 作 fallback；4GB 箱可、2GB 箱紧、CPU 慢但免费无限）。**MT 改 `deepl` 优先**（已备 key、500k 字/月）**以省下 CF 10k neurons 全给 ASR**；TTS 仍 piper 本地（不吃 neurons）。全局分钟池默认 100–120/天仍在 CF 214 内、保守可行。**Groq 注册成功后即恢复其为 ASR 主力**（容量大幅回升）。
>
...
| `asr_default`（v3.3 云优先） | `groq`(whisper-large-v3-turbo)→`cloudflare`→`faster_whisper`(仅 cli/self-host) | hosted 不烤 faster-whisper；配额尽→`free_pool_exhausted`。**临时（Groq 注册受阻 2026-06）：CF 主 + 本地 faster-whisper 兜底（hosted 临时 re-bake）+ MT 改 DeepL 优先省 neurons，见 §5；Groq 后补恢复主力** |
