...
>
> **Groq 地缘受限 → 降为可选、CF+本地为可靠主力（2026-06 实证）**：Groq **封锁中国大陆区 + 标记多数机房/VPN IP**（中国直连 403 Forbidden，代理 signup error，云主机 Forbidden）。**不仅卡注册，运行时 worker（VPS 机房 IP）也可能连不上 Groq API** → 对"中国运营者 + VPS worker"这套组合天生脆弱。**故 Groq 降为「可选/尽力而为」（IP 能通才用），不作依赖。** **可靠主力 = 地缘干净的栈**：ASR **`cloudflare`(Workers AI Whisper) 主**（~214 audio-min/天，CF 上跑无地区封）→ **`assemblyai`（免费配额、secret `ASSEMBLYAI_API_KEY`）兜底**（免费层有额度上限，超额 fail-to-error）→ **`faster_whisper` 本地末位**（hosted **重新 bake** 作 fallback；4GB 箱可、2GB 箱紧、CPU 慢但免费无限）；**MT 改 `deepl` 优先**（已备 key、500k 字/月）以省下 CF 10k neurons 全给 ASR；TTS piper 本地（不吃 neurons）。**诚实容量**：无可靠 Groq，免费 ASR 日上限 ≈ CF 214 + AssemblyAI 免费配额 + 本地；默认分钟池 100–120/天在此内、保守可行，但**无 Groq 大头寸、扩容靠本地或 BYOK/付费**。**若启用 Groq：须用干净住宅 IP 注册 + 先在 worker 箱（建议 Hetzner 德国）实测 API 可达再纳入。**
>
...
| `asr_default`（v3.3 云优先） | `groq`(whisper-large-v3-turbo)→`cloudflare`→`assemblyai`（免费配额、secret `ASSEMBLYAI_API_KEY`）→`faster_whisper`(仅 cli/self-host) | 配额尽→`free_pool_exhausted`。**Groq 地缘受限（封中国区+机房IP，2026-06 实证）→ 降为可选/尽力而为；可靠主力 = CF 主 → AssemblyAI 兜底 → 本地 faster-whisper 末位（hosted re-bake）+ MT 改 DeepL 优先省 neurons，见 §5** |
