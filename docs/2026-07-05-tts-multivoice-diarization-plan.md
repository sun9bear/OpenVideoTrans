...
3. **CF MeloTTS = 纳入** ✅ **token 走 Cloudflare Secrets Store**（业主已选此路径，与 R2/DeepL 一致）：业主在现有 store（`store_id 78d1fc96c03044c2bf20f9dce7a47340`）建 secret `CF_AI_API_TOKEN`（scope Workers）；**account id 非密钥、复用现有 `R2_ACCOUNT_ID`**。P3b 接线（镜像 `SS_DEEPL_API_KEY`）：wrangler.jsonc `secrets_store_secrets` 加 `{binding:"SS_CF_AI_API_TOKEN", store_id, secret_name:"CF_AI_API_TOKEN"}` + core.ts `RawEnv` 加 `SS_CF_AI_API_TOKEN?` + `resolveSecrets()` 映射到 `env.CF_AI_API_TOKEN`；`collectCredentials`（credentials.ts）**已**读 `CF_AI_ACCOUNT_ID`+`CF_AI_API_TOKEN`→`providers.cloudflare`→worker，内核无需改。**红线：仅 Workers FREE 计划才注入**（否则超 10k neurons/天静默计费、违 §1）。更新 DEPLOY-RUNBOOK（原写 plain `wrangler secret put CF_AI_*`）。**前置状态（2026-07-05 收尾）：token 已存入 Secrets Store（`CF_AI_API_TOKEN`、Active、Workers scope、同一部署账号）✅；账号 = Workers **FREE** 计划（业主确认）✅；接线**已完成**（分支 `feat/tts-cf-secrets-wiring`：wrangler.jsonc 绑 `SS_CF_AI_API_TOKEN` + `CF_AI_ACCOUNT_ID` var、core.ts `resolveSecrets` 映射、`secrets.test.ts` 4 测；286/286 vitest + tsc 通过）✅；egress = **空操作**（真机 nftables 只有 Docker 默认链、无自定义白名单，worker 容器已能直连 `speech.platform.bing.com` 与 `api.cloudflare.com`——无需改动;将来若启用 egress 兜底须把这两域名加白名单）✅。** 顺带：store 里的 `ASSEMBLYAI_API_KEY` 是付费 ASR（红线 §1）——**永不绑定 / 永不进免费池**（wrangler.jsonc 已注明刻意不绑）。
   - **多透镜对抗审核发现（已修，同 PR #82）**：CF 单凭据对同时启用 CloudflareTTS/**MT**/**ASR**；MT 路由无 commercial-safe 门且 CF 原居 `AUTO_LADDER['mt']` 之首 → 接 CF token（本为 TTS）会**静默把 CF m2m100 变成全队默认翻译引擎**（未审行为变更 + CF-MT 对 'auto' 源硬失败白选+reroute）。**修复（项目主拍板"改阶梯顺序"）**：`ladder.py` MT 改 `groq→deepl→ollama→cloudflare`（CF 末位回退），新增 `test_mt_ladder_keeps_cloudflare_as_last_resort` 守护,母文档 §89 已注调整。ASR 保持 T1.2 云优先（`groq→cloudflare→faster_whisper`,有测试守护,未动）。

4. **edge_tts 商用门 = 放开给"用户显式选择"（业主自担微软 ToS/许可风险）** ✅ 已拍板（2026-07-05）。背景：`COMMERCIAL_SAFE_TTS = {piper, cloudflare}`——edge_tts 被刻意排除在**自动路由**外（微软 read-aloud ToS 限非商用，AD-6 非商用实验 lane）；但免费引擎里**只有 edge_tts 有中文男声**（piper zh=huayan 女、CF MeloTTS zh=单女声），业主的核心诉求（中文男声/真选择）非 edge 不可。决策：**放开 commercial-safe 门，但仅限用户在 picker 里显式 pin 的 edge 音色**；业主像 AIGC 一样自担 ToS/许可风险。**护栏（实施须落实）**：(i) edge **绝不进自动默认/自动 ladder**——`_available_free_providers` 对 tts 仍只交 `_locale_commercial_safe_tts`（= {piper,cloudflare}），自动路由行为**不变**；(ii) 仅当 `JobPlan.tts_voice`（用户显式 pin）解析到 edge 音色时，**pin 越过商用门**用 edge 配音（落在既有 pin 机制上——`base.select()` 认 pin；但注意 `pipeline.py:445-446` 现**无条件覆盖** `plan.tts`，P0 软-pin 必须改成"有显式 pin 则不覆盖"）；(iii) 前端 picker 对 edge 音色**标注"实验/非商用"**；(iv) edge 仍非商用输出的**默认**。

**最终实施范围：P0（能力清单+软-pin，含"显式 pin 越过商用门"）+ P1（Piper 男女声+选择器）+ P2（edge_tts adapter + 装 edge-tts 到镜像）+ P3b（CF MeloTTS，接线已完成 ✅）+ P4（diarization + per-speaker voice_map）；P3（Kokoro）延后。** 均走 `ship-unit`（worker 先行、片B/P4 完整重建镜像）；CodeX 两审级约 2026-07-09 前 OpenAI 用量封顶 → 用对抗多透镜 workflow + CI 兜。**前置（业主）：CF token 注入 ✅、Workers FREE 确认 ✅、egress 空操作 ✅——前置已全部就绪。**
