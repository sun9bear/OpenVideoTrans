...
| 1 | Track B Tier 1 MVP 实施方案 | B | 新 repo 目录 + CF Pages/Workers/R2/D1/Queues + Python media worker + free-video-dub 移植范围 + 上传/排队/执行/下载闭环 + 24h TTL + abuse gate + **最小内嵌单 lane 队列**；**不含** BYOK / 付费 / 高质量核心迁移 / 完整多 lane 调度器 | ✅ v3 已多 agent 复审 + CodeX 修订（待终审锁定） | [本 repo 子方案](2026-06-20-track-b-tier1-mvp-implementation-plan.md) |
