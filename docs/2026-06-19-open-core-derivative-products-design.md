...
| 1 | Track B Tier 1 MVP 实施方案 | B | 新 repo 目录 + CF Pages/Workers/R2/D1/Queues + Python media worker + free-video-dub 移植范围 + 上传/排队/执行/下载闭环 + **输出模式（字幕/配音）** + 24h TTL + abuse gate + **单 lane 优先队列（SPT+aging+预留槽位）**；**不含** BYOK / 付费 / 高质量核心迁移 / 完整多 lane WFQ 调度器 | ✅ v3.3 执行基线（v3.2 多 agent + CodeX 两轮锁定 + 项目主 v3.3 增量：字幕模式 / 云 ASR / 模式感知调度 / 透明度） | [本 repo 子方案](2026-06-20-track-b-tier1-mvp-implementation-plan.md) |
