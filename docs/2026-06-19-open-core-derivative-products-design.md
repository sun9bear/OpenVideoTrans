...
| 1 | Track B Tier 1 MVP 实施方案 | B | 新 repo 目录 + CF Pages/Workers/R2/D1/Queues + Python media worker + free-video-dub 移植范围 + 上传/排队/执行/下载闭环 + **输出模式（字幕/配音）** + 24h TTL + abuse gate + **单 lane 优先队列（SPT+aging+预留槽位）**；**不含** BYOK / 付费 / 高质量核心迁移 / 完整多 lane WFQ 调度器 | ✅ v4 执行基线（v3.2 多 agent + CodeX 两轮锁定 → v3.3 增量：字幕模式 / 云 ASR / 模式感知调度 / 透明度 → v4 CodeX 三轮收口 & 语言精化） | [本 repo 子方案](2026-06-20-track-b-tier1-mvp-implementation-plan.md) |
...
| 基础 Web / worker 框架 | **open** | 薄控制面 + worker + 队列骨架（不含托管调度策略）；**具体技术栈由子方案定**——如 Track B hosted = CF Workers + Svelte，self-host/cli = 薄 HTTP（FastAPI 等）；本表不钉死单一框架（CodeX P2.3） |
