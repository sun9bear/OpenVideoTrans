...
| 2026-06-24 | T1.1（autodub-core 移植：`config`/`ffmpeg_utils`/`stages` 拷改进 `packages/autodub-core`；复用 `ovt_schemas` 契约；provider seam 依赖注入；`ingest` 仅本地文件、URL/yt-dlp 推迟 T1.4） | feat/t1.1-autodub-core | _（PR 待建）_ | Claude 4 维自审（0 blocking，2 候选对抗驳回，5 minor 吸收）+ CodeX CLI **11 轮**（**P1 红线**：`run_pipeline` 去 `allow_paid` 调用方入口、内核恒传 False；+ ~16 P2 resumability/原子性：tts/mux 缓存门、ingest 同文件/旧后缀/陈旧音频、TTS raw 原子、`atomic_output` 统一所有媒体写、align/tts 元数据持久化早于 skip-gate、demucs ambient 先写；+ 1 P3 demucs 临时目录泄漏）→ 收敛 | 本地全绿（88 过 + 5 xfail；含 e2e smoke 真出 mp4+srt；golden 守 `assign_timing`/`atempo_chain_for`/`stitch_timeline` 帧级/SRT）；ruff/pyright/codegen-diff 净 | T1.2 ∥ T1.3a–g |
...
- **进行中：** T1.1（autodub-core 移植）——实现 + 4 维自审 + **CodeX CLI 11 轮收敛**（P1 红线 + ~16 P2 resumability/原子性 + 1 P3 全修）+ 本地全绿（88 过 + 5 xfail，含 e2e smoke 真出 mp4+srt）+ ruff/pyright/codegen-diff 净；待 **push → PR → @CodeX 终审 → 里程碑内合并**。分支 `feat/t1.1-autodub-core`（off `29e64b6`）。
